...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const { name, email, dob, password, aadharNumber,  mobileNumber, state, district, cityName, pincode, doorNumber } = req.body

  if (!name || !email || !dob || !password || !aadharNumber || !mobileNumber || !state || !district || !cityName || !pincode || !doorNumber) {
    res.status(400)
  ...
  const user = await User.create({ name, email, dob, password: hashedPassword, aadharNumber, mobileNumber, state, district, cityName, pincode, doorNumber })
